...
const LuxuryLabelComponent1 = ({ className = "", favoriteIcon, onPriceUpdate }) => {
  ...
  useEffect(() => {
    ...
    onPriceUpdate(newTotalPrice); // Notify parent component about price update
  }, [quantity, unitPrice, onPriceUpdate]);
  ...
  const onLuxuryLabelContainerClick = useCallback(() => {
    navigate("/product-view");
  }, [navigate]);

  return (
    <div
      className={`luxury-label ${className}`}
      onClick={onLuxuryLabelContainerClick}
    >
      <img
